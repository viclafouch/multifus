...
Le titre d'une fenêtre ramené au seul pseudo, `Alpha` au lieu de `Alpha - Dofus Retro v1.48.21`. C'est ce que la barre des tâches de Windows montre, et six clients y deviennent lisibles d'un coup d'œil. **Windows seul** : le client de macOS garde son titre quoi qu'on lui demande, et [perimetre.md](./docs/perimetre.md) en fait un refus. Réglage global, décoché par défaut ; décocher, comme quitter multifus, rend à chaque fenêtre le titre que Dofus lui avait écrit. Il ne cache rien à multifus, qui reconnaît un titre court et y lit le pseudo sans avoir eu besoin de se souvenir de l'avoir écrit : rien d'autre dans l'application ne change de comportement, un relancement compris.
