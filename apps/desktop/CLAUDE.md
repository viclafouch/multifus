...
navigateur piloté suffit si `window.__TAURI_INTERNALS__` et
`window.__TAURI_OS_PLUGIN_INTERNALS__` sont bouchonnés avant le chargement de la
page : `@tauri-apps/api` n'appelle que son `invoke`, et le plugin `os` lit la
version du système à même la page. L'instantané bouchon doit porter les vraies
formes, une valeur inventée cassant l'écran sans rien dire.
