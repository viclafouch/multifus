...
| `apps/website/src/styles.css`         | La palette du site, le remappage des jetons de `retro.css`, le fond vivant, et les matières que le site seul emploie : `glass`, `canopy`, `mast`, `tab`, `card`, `mark`, `surface`, `reveal`                                                             |
| `apps/website/src/components/`        | Le cadre d'une page : `SiteShell`, `SiteHeader`, `MastLink`, `Cartouche`, `LanguageOffer`, `Band`, `DecorBand`, `Glint`, `WayLink`, `PageCard`, `Opening`, `ProseBlock`, `PlateBlock`, `RivalTable`, `MarkKey`, `MarkTip`, `RuneTable`, `ProvenanceList` |
