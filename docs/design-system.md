...
| `apps/website/src/styles.css`         | La palette du site, le remappage des jetons de `retro.css`, le fond vivant, et les matières que le site seul emploie : `glass`, `canopy`, `mast`, `tab`, `card`, `mosaic`, `vignette`, `mark`, `surface`, `reveal`                                                   |
| `apps/website/src/components/`        | Le cadre d'une page : `SiteShell`, `SiteHeader`, `MastLink`, `Cartouche`, `LanguageOffer`, `Band`, `DecorBand`, `Glint`, `WayLink`, `Vignette`, `PageCard`, `Opening`, `ProseBlock`, `PlateBlock`, `RivalTable`, `MarkKey`, `MarkTip`, `RuneTable`, `ProvenanceList` |
...
**Le site garde cette palette, et c'est un choix qui a coûté un aller-retour.**
Une encre bleu-nuit avec un jade d'accent a été posée puis retirée le même jour :
elle était plus moderne, et elle ne ressemblait plus à rien du jeu, ni à Multifus.
Un site de fan qui perd les couleurs de son jeu perd ce qu'il avait de propre, et
plusieurs concurrents tiennent déjà ce sombre-froid-accent-vif qu'on voit partout.
Ce qui manquait n'était pas la palette, c'était l'air : des marges plus larges,
un titre qui dit une phrase au lieu d'un nom, et une capture qui déborde au lieu
d'être bornée au milieu.

Le site tire donc deux choses en propre du fond uni. Un ton, `--deep`, un cran
sous l'`iron`, qui donne un relief là où la fenêtre avait un décor. Et ses
matières à lui, parce que celles de `retro.css` sont écrites pour du verre posé
sur un décor : `plate`, `note` et `hood` mélangent de l'`iron` avec du vide, donc
sur un fond de la même couleur elles deviennent invisibles. `glass`, `canopy` et
`card` font le même travail sur du plein.

Le site élargit aussi `--container-world`, la seule mesure de `retro.css` qu'il
repointe. Une fenêtre de 320 points et une page lue sur un écran de bureau n'ont
pas la même respiration, et `--page` porte la sienne.

Quand une matière manque au logiciel, on retourne à la source plutôt que de
l'inventer.
...
1. **Dans le logiciel**, le vert dit l'action du moment et ce qui est en place,
   rien d'autre. Un seul bouton vert par map, jamais un titre, jamais un
   avancement
2. **Sur le site**, l'action porte un nom, « Télécharger », et le vert habille
   d'abord elle. Elle se répète sans se disputer : la barre du haut la porte en
   permanence, un écran en porte une, et `/telecharger` en porte deux, le Mac et
   Windows. C'est la même action à chaque fois, coupée par le système ou reprise
   plus bas, jamais deux actions qui se battent. Sur `/telecharger`, le bouton de
   la barre tombe en `slate` : il mènerait à la page où l'on est déjà. Le vert
   sert aussi de repère de lieu, le filet sous un titre de bande, le trait sous
   l'onglet où l'on est, et le second membre du titre de l'accueil : là il ne se
   clique pas, il dit où l'œil en est
3. **Le comparatif porte trois couleurs**, et c'est la seule table qui en porte :
   la feuille pour ce qui est fait, l'ambre pour ce qui l'est à moitié, la flamme
   pour ce qui ne l'est pas. Ce sont les trois couleurs d'état du jeu, pas trois
   teintes inventées. La règle d'avant refusait la couleur, de peur que
   soixante-douze cases vertes noient le bouton du bas ; ce qui les noyait,
   c'était de les peindre du même vert plein que lui. Ici le vert ne remplit
   qu'un disque de vingt-deux pixels, et il reste un glyphe. La forme suffit
   seule, disque plein, demi-disque, anneau vide, parce qu'un daltonien lit la
   forme, un lecteur d'écran lit le mot, et le contraste forcé garde le tracé du
   SVG là où un fond CSS aurait tout aplati
4. L'or et l'ambre ne vivent que dans le logo du jeu. Ils survivent aux deux
   endroits que `CONTEXT.md` nomme, la part sans couleur de la roue et l'étoile
   du principal, et nulle part ailleurs
5. Les deux sceaux de sexe gardent leur couleur, seule exception au vert : ils se
   choisissent côte à côte, et deux disques verts ne se distinguent plus
6. Ce qui n'est pas cliquable ne porte ni fond ni bord. Un chemin de réglage est
   du texte, un état est un point de couleur suivi d'une phrase

### Le décor et le verre

7. Une plaque est de l'`iron` transparent, jamais du `slate`. La couleur du voile
...
8. Le texte posé sur du verre est gravé par `--engrave`. Ce que le joueur tape ne
   se grave pas
9. On assombrit local et le moins possible. Un voile qui cache le décor coûte
   plus qu'il ne rapporte : c'est le décor qui dit où on est
10. Le décor ne s'arrête jamais de bouger, celui qui sort compris. Une panoramique
    qui ne tourne que sur l'image affichée fait sauter l'image au changement
11. Une ombre qui s'éteint le fait en courbe, jamais en droite. Une rampe de trois
...
12. Une map occupe la fenêtre entière, et le reste flotte au-dessus, hors du flux.
...
13. Ce qui flotte a son ombre, `Shade` en haut et en bas, et un écran commence et
    finit hors d'elle : c'est `--spacing-fall` qui donne les deux marges. Le
    texte qui glisse dessous s'efface au lieu de se cogner
14. Le titre d'une map se pose toujours à la même hauteur, quelle que soit celle
...
15. Un écran garde une marge de hauteur : les fontes chargent en `swap`, et la
    page grandit de quelques pixels le temps que les vraies arrivent
16. Un dialogue tient sa place de son composant, jamais de sa classe. Une classe
...
17. Rien n'est monté ni démonté au fil d'une animation. Tout est là à la première
...
18. Une animation qui fait attendre ne se pose que là où on ne passe qu'une fois.
    La mise en route a ses quatre secondes ; une map s'ouvre en 800 ms sans rien
    bloquer
19. Les délais s'annulent sous `prefers-reduced-motion`, pas seulement les durées
20. Une translation d'entrée vit dans un conteneur qui coupe, sinon elle agrandit
    la zone défilable et la barre paraît une demi-seconde
21. Un survol change une couleur, et un clic aussi. Rien ne bouge de place sous
    ...
    même de l'écran. Le site ne le reprend que là où il est ce même menu, et il
    ne l'invente jamais : c'est à cette condition que l'exception reste unique.
    Sur le fond uni du site, le voile de `btn-way` ne se voit plus, faute de
    décor dessous ; ce qui reste, et qui suffit, c'est le losange qui entre et
    le titre qui s'éclaircit. Les six fonctionnalités de l'accueil le portaient
    et ne le portent plus : elles sont devenues des vignettes, parce que six
    titres et six phrases l'un sous l'autre ne donnaient rien à voir quand on a
    une capture par fonctionnalité. Une vignette est une `card`, elle vit donc
    du survol de la `card`, un filet de jade en haut et l'affiche qui remonte à
    sa pleine valeur, et rien ne se déplace
22. Une tête du dolmen n'apparaît ni ne s'efface d'un coup. Celle qui arrive
...
23. Chaque `@utility` est complète et ne partage aucune base. Tailwind v4 ne les
    émet pas dans l'ordre du fichier : deux qui posent la même propriété se
    battent, et c'est le hasard qui gagne
24. On n'écrit pas une matière avant l'écran qui l'emploie. Une `@utility` sans
    appelant est une abstraction pour plus tard
25. Aucune valeur en dur dans un composant : un jeton, ou rien. Ce qui se pose sur
    une tête se déduit de `--head`, qui rétrécit quand le roster grossit
26. Une taille de texte du thème s'ajoute à `cn`, dans `packages/retro/src/cn.ts`.
...
27. Aucun `backdrop-filter` au-dessus d'une vidéo. Une vidéo qui joue est
...
28. Un trait fin tient sa propre couche, `transform: translateZ(0)`. Une ligne
...
29. Un bouton est du texte en Bebas capitales. Pas d'icône à côté d'un mot ; ce
    qui reste de `lucide` est un glyphe seul, et s'en ira
30. Bebas est la fonte de Multifus. Ce que le joueur a écrit reste en Roboto, dans
    sa casse à lui
31. Bebas est déclarée dans `retro.css` et non plus importée de `fontsource`,
...
32. Bebas ne porte pas une phrase. Bebas Neue n'a pas de bas de casse, et ce
...
33. Un corps de texte commence par un verbe, et ne parle pas par métaphore quand
    il y a un geste à faire
34. Un titre nomme le réglage comme le système l'écrit, et un mot que le joueur
    lira sur son propre écran passe entre guillemets par `quoted`
35. Ce qui s'apprend une fois s'ouvre dans un dialogue, pas dans la plaque qui le
...
36. Une liste qu'on ne lit que le jour où ça coince se plie, une entrée ouverte
...
37. Ce qu'une page du site ne fait pas se lit en intertitre, jamais en note de bas
...
38. La langue se change par son drapeau, en haut à droite, et le drapeau mène à
...
39. `Screen` si c'est une liste, `StageScreen` si ça montre le jeu. Les deux
    posent `MapHeader`, donc le titre, le fronton et une phrase
40. `Panel` par groupe, c'est le verre. `FieldRow` pour une ligne de réglage,
    `Tick` pour ce qui s'allume, `Button` pour un geste
41. Le nom de la map et son décor se déclarent dans `constants/world.ts`, et
    `CurrentMap` la branche
42. Ni onglet ni barre latérale : on vient de l'accueil, on y retourne par
    « Retour » ou par Échap
