...
**Le collage des réponses rapides, mesuré le 24 août 2026 avec un binaire jetable.** Quatre réponses, et celles de Windows sont dans [windows.md](./windows.md) : aucune ne se transporte. La combinaison arrive dans le chat, `Super+V` par `CGEventPost`, tap HID, source privée. Les modificateurs physiques n'ont pas à être relâchés d'abord, `Control` et `Shift` tenus le texte arrive quand même, et c'est la source privée qui l'obtient. Le presse-papiers ne peut pas être rendu avant **50 ms**, à 10 ms le champ reste vide, d'où `GIVE_BACK_AFTER` à 150 dans `app::quick_replies`. La quatrième, le chat doit-il déjà avoir le focus, n'a jamais été mesurée ici ; elle l'a été sur Windows et la réponse y est non.

**Le binaire jetable emprunte l'autorisation de son lanceur.** `CGEventPost` demande l'Accessibilité, et en développement c'est le terminal qui la porte, jamais multifus. Un essai lancé depuis un terminal non autorisé échoue sans rien dire, ce qui se lit comme « Dofus n'accepte pas le collage ». La sonde est supprimée, la remarque vaut pour la suivante.
...
**`AXTitle` accepte l'écriture, répond succès, et ne renomme rien.** Mesuré le 25 août 2026, macOS 26.3, client Retro 1.49.1 en jeu, autorisation accordée : `osascript -e 'tell application "System Events" to tell (first application process whose bundle identifier is "com.dofus.d1elauncher") to set value of attribute "AXTitle" of window 1 to "Alpha"'` ne lève aucune erreur, et le titre relu une seconde plus tard est toujours `Prune-lf-[ART] - Dofus Retro v1.49.1`. Le reste de la chaîne est sain, ce qui rend le piège invisible : le bundle est le bon, `AXWindows` est peuplé, le titre est lisible. Le client est le launcher Electron d'Ankama, et Chromium possède la barre de titre de son `NSWindow` ; les setters d'accessibilité y sont des coquilles vides. Aucun chemin de secours : aucune API publique ne renomme la fenêtre d'un autre processus, et Mission Control lit son étiquette au serveur de fenêtres.

**Ce qui a coûté la soirée, c'est d'avoir attendu un refus.** `set_title` triait `InvalidUIElement`, `CannotComplete` et le reste pour écrire une ligne de journal honnête, et le seul cas qui se présente est celui qu'il ne pouvait pas voir : `kAXErrorSuccess` sans effet. Six clics dans les Paramètres, zéro ligne au journal, zéro caractère changé à l'écran. **La leçon vaut pour toute écriture par Accessibility : le succès de l'appel ne prouve rien, seule la relecture prouve.** Le refus est dans [perimetre.md](./perimetre.md), et la moitié macOS du titre court est partie avec.
